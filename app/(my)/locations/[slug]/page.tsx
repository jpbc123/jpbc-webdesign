...
//
// City pages are Malaysia-only: a market opts in with `pages.locations` in
// its config once it has its own city data.
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!loc) return {};
  return {
    title: `Web Designer in ${loc.name} - ${setup} Down, ${monthly}/month`,
    description: `Hand-coded custom websites for ${loc.name} small businesses. ${setup} down, ${monthly}/month including domain, hosting, and unlimited edits. Serving ${loc.areas.slice(0, 3).join(", ")} and beyond.`,
...
export default async function LocationPage({ params }: Props) {
  ...
  const pageFaqs = [...loc.faqs, ...pricingFaqs(market).slice(0, 3)];
  ...
  return (
    <>
      ...
      <section className="mx-auto max-w-4xl px-6 pb-4 pt-16">
        ...
        <div className="mt-8">
          <WhatsAppButton market={market} message={`Hi JPBC! I run a business in ${loc.name} and I'm interested in a website.`}>
