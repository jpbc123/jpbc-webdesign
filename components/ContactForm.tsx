...
import { isValidPhone } from "@/lib/market";
...
export default function ContactForm({ market }: { market: Market }) {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!isValidPhone(market, data.phone)) {
      setPhoneError(market.phone.error);
      return;
    }
    setPhoneError("");
    setStatus("sending");
    ...
    try {
      const res = await fetch("/api/leads", {
        ...
        body: JSON.stringify({ type: "contact", market: market.code, ...data }),
      });
  ...
  return (
    <form onSubmit={onSubmit} className="mt-6 space-y-4">
      <div>
        ...
        <input id="c-name" name="name" required maxLength={100} className={inputCls} placeholder={market.voice.formNamePlaceholder} />
      </div>
      <div>
        <label htmlFor="c-business" className="mb-1 block text-sm font-semibold text-ink">Business name</label>
        <input id="c-business" name="business" required maxLength={150} className={inputCls} placeholder={market.voice.formBusinessPlaceholder} />
      </div>
      <div>
        <label htmlFor="c-phone" className="mb-1 block text-sm font-semibold text-ink">{market.phone.label}</label>
        <input id="c-phone" name="phone" type="tel" required className={inputCls} placeholder={market.phone.placeholder} aria-describedby={phoneError ? "c-phone-err" : undefined} />
