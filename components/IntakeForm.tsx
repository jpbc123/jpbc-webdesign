...
import { fillBusiness, isValidPhone, price, waLink } from "@/lib/market";
...
export default function IntakeForm({ market }: { market: Market }) {
  ...
  const packageOptions = [
    {
      value: "monthly",
      label: `Monthly — ${price(market, market.pricing.setup)} down, ${price(market, market.pricing.monthly)}/month`,
      hint: "Everything included. Most popular.",
    },
    {
      value: "lumpsum",
      label: `Lump sum — from ${price(market, market.pricing.lumpSum)}`,
      hint: `Own it outright + ${price(market, market.pricing.lumpSumCare)}/month care.`,
    },
  ...
  async function submit() {
    setError("");
    if (!isValidPhone(market, answers.phone)) {
      return setError(market.phone.error);
    }
    setStatus("sending");
    try {
      const res = await fetch("/api/leads", {
        ...
        body: JSON.stringify({ type: "get-started", market: market.code, ...answers }),
      });
  ...
  if (status === "sent") {
    return (
      <div className="rounded-3xl border-2 border-accent/40 bg-surface p-8 text-center">
        <p className="display text-3xl text-ink">{fillBusiness(market.voice.intakeThanks, answers.business)}</p>
        <p className="mt-3 text-muted">
          We&apos;ve got your details and will WhatsApp you shortly. Want to skip the queue? Message us first:
        </p>
        <a
          href={waLink(market, `Hi JPBC! I just filled the get-started form for ${answers.business}.`)}
          target="_blank"
      ...
      {step === 0 && (
        <fieldset>
          ...
          <input
            ...
            placeholder={market.voice.formIntakeBusinessPlaceholder}
      ...
      {step === 4 && (
        <fieldset>
          ...
          <input
            ...
            aria-label={`WhatsApp number (${market.label})`}
          />
        </fieldset>
      )}
